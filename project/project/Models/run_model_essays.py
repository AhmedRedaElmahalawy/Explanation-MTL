# -*- coding: utf-8 -*-
"""run_model_Essays.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gw4fMInRffKAZKYqNv-ei8bHZSwzAbCs
...
data = pd.read_csv('/content/project/project/dataset/Essays/essays_preprocessing.csv')
...
embeddings = joblib.load('/content/project/project/dataset/Essays/bert_embeddings_essays.pkl')
...
class CNNExpert(nn.Module):
    def __init__(self, input_dim, num_conv_layers=4, filters=64, kernel_size=3, pool_size=3, dense_units=64, dropout=0.01):
...
# Define labels for each task
y = np.column_stack([labels[col] for col in ['cEXT1', 'cNEU1', 'cAGR1', 'cCON1', 'cOPN1']])
...
def train_model(model, X_train, y_train, X_val, y_val, loss_fn, optimizer, batch_size, epochs, early_stopping, scheduler):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batches):
            ...
            epoch_loss += loss.item()
        val_loss = validate_model(model, X_val, y_val, loss_fn)
        # print(f"Epoch [{epoch+1}/{epochs}] Loss: {epoch_loss / num_batches:.4f}, Val Loss: {val_loss:.4f}, LR: {scheduler.get_last_lr()[0]:.6f}")
        if epoch == 0:
            with torch.no_grad():
                print("--------------------")
        early_stopping(val_loss, model)
...
# Compute y_pred_labels using the optimal thresholds
y_scores_test = model(X_test).detach().numpy()  # Get the raw predictions from the model
...
# Prepare Expert Outputs for Gating Net
# ========================================
def get_expert_outputs(X, svc_experts, num_experts):
    outputs = np.zeros((X.shape[0], 5, num_experts))
    for task_idx in range(5):
        for expert_idx in range(num_experts):
...
svc_experts = [[SVCExpert(random_state=R) for _ in range(num_experts)] for _ in range(5)]  # 5 tasks × N experts
for task_idx in range(5):
    for expert_idx in range(num_experts):
        svc_experts[task_idx][expert_idx].train(X_train, y_train[:, task_idx])

# Get expert outputs for train/val/test
expert_outputs_train = get_expert_outputs(X_train, svc_experts, num_experts)
expert_outputs_val = get_expert_outputs(X_val, svc_experts, num_experts)
expert_outputs_test = get_expert_outputs(X_test, svc_experts, num_experts)
...
train_model(model, X_train_tensor, expert_outputs_train, y_train_tensor,
            X_val_tensor, expert_outputs_val, y_val_tensor,
            loss_fn, optimizer, scheduler, epochs, batch_size)
...
with torch.no_grad():
    ...
    acc = accuracy_score(y_test, preds_bin) * 100
    f1_micro = f1_score(y_test, preds_bin, average='micro')
    f1_macro = f1_score(y_test, preds_bin, average='macro')
...
y_pred_labels = preds_bin
y_test_tensor = torch.tensor(y_test, dtype=torch.float32)
y_pred_labels_tensor = torch.tensor(y_pred_labels, dtype=torch.float32)

tp, fp, fn = 0, 0, 0
num_tasks = 5
for i in range(num_tasks):
    tp += torch.sum((y_test_tensor[:, i] == 1) & (y_pred_labels_tensor[:, i] == 1)).item()
    fp += torch.sum((y_test_tensor[:, i] == 0) & (y_pred_labels_tensor[:, i] == 1)).item()
    fn += torch.sum((y_test_tensor[:, i] == 1) & (y_pred_labels_tensor[:, i] == 0)).item()
...
if f1_micro_sklearn > 0.67:
    ...
    print(f"F1-micro (sklearn): {f1_micro_sklearn:.4f}")
    print(f"F1-macro (sklearn): {f1_macro_sklearn:.4f}")
    ...
    for i in range(num_tasks):
        ...
        print(f"Task {labels_name[i]} - F1-Score: {f1:.4f}")

    print(f"F1-micro (manual): {f1_micro_manual:.4f} vs F1-micro (sklearn): {f1_micro_sklearn:.4f}")
    print(f"F1-macro (manual): {f1_macro_manual:.4f} vs F1-macro (sklearn): {f1_macro_sklearn:.4f}")

R = 6:
Manually calculated F1-micro: 0.6981
F1-micro (sklearn): 0.6981
Manually calculated F1-micro: 0.6981
Manually calculated F1-macro: 0.6975
F1-micro (sklearn): 0.6981
F1-macro (sklearn): 0.6975
Task cEXT1 - F1-Score: 0.7071
Task cNEU1 - F1-Score: 0.7018
Task cAGR1 - F1-Score: 0.7301
Task cCON1 - F1-Score: 0.6984
Task cOPN1 - F1-Score: 0.6501
F1-micro (manual): 0.6981 vs F1-micro (sklearn): 0.6981
F1-macro (manual): 0.6975 vs F1-macro (sklearn): 0.6975